...
<script>
import {getUnUserList,addGroupRelUserList} from "../../../api/authority"
export default {
  name: "",
  components: {},
  props: {},
  ...
  created() {},
  mounted() {
    getUnUserList(this.$route.query.groupId).then(data=>{
      console.log(data.data);
      for (let i = 0; i < data.data.length; i++) {
          this.options.push({label:data.data[i].userName,value:data.data[i].id})
      }
    },error=>{
  ...
  methods: {
    ...
    add(){
      addGroupRelUserList({groupId:this.$route.query.groupId,id:this.selValue})
      // this.$router.go(0)
      this.mounted()
    },
    // 全选
